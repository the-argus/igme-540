...
auto Mesh::ReadOBJ(const wchar_t* filename) noexcept -> std::tuple<unique_vertices_t, unique_indices_t>
{
	...
	return std::make_tuple<unique_vertices_t, unique_indices_t>(std::move(verts), std::move(indices));
}
